// run_step_10_validation.js -- BA validation matrix runner (Step 10.2 Wave 3)
// Runs 9 primary scenarios x 30 trials x 5000 iterations against VB6 BA reference.
// Exports VB6-compat and rich CSVs; prints verdict summary.
// Mirrors run_step_9_5.js structure -- substitutes HCA -> BA at call sites.
...
var OUT_ROOT = path.join(BACKEND_DIR, 'out', 'step_10');
...
console.log('========================================');
console.log('Step 10.2 -- BA Validation Matrix');
console.log('9 scenarios x 30 trials x 5000 iter');
console.log('Algorithm:   Bisection Algorithm (BA)');
...
for (i = 0; i < scenarios.length; i++) {
  ...
  // 1) Run Node.js BA optimization
  var nodeResult;
  try {
    nodeResult = integration.runScenarioBA(sc.H, sc.fc, {
      numTrials: 30,
      maxIterations: 5000,
    ...
    console.log('    ERROR runScenarioBA: ' + err.message);
    verdicts.push({ H: sc.H, fc: sc.fc, mode: '-', pass: false, error: err.message });
    console.log('');
    continue;
  }

  // 2) Load VB6 BA reference (algo="BA" -- added in Wave 1)
  var vb6Ref;
  try {
    vb6Ref = integration.loadVB6Reference(sc.H, sc.fc, VB6_DIR, 'BA');
  } catch (err) {
  ...
  // 3) Compare (deep for fc=280, smoke otherwise) -- identical to Step 9.5
  ...
  // 4) Export CSVs with BA in filename
  var vb6OutPath = path.join(OUT_VB6,
    'loopPrice-BA-H' + sc.H + '-' + sc.fc + '.csv');
...
if (anyDeferred) {
  ...
  console.log('  * = bestCostMatch deferred to extended run ' +
    '(may need scripts/extended_run_ba.js -- Wave 4 decision)');
}
console.log('');
...
// Persist verdicts for Wave 4 report generation
