...
//
// Public surface:
//   wilcoxonSignedRank(samplesA, samplesB)   -> { W, z, pValue, n,
//                                                  Wplus, Wminus,
//                                                  conclusion, note }
...
// Wilcoxon signed-rank, two-sided, with average-rank tie handling and
// continuity correction on the normal approximation. Designed for
// n >= 6; for very small n the p-value is approximate (note returned).
function wilcoxonSignedRank(samplesA, samplesB) {
  ...
  // Continuity-corrected z (matches scipy.stats.wilcoxon default).
  var z;
  if (sigma === 0) {
    z = 0;
  } else {
    var diff = W - mu;
    var corr = diff > 0 ? -0.5 : (diff < 0 ? 0.5 : 0);
    z = (diff + corr) / sigma;
  }

  var pValue = 2 * normalCDF(-Math.abs(z));
